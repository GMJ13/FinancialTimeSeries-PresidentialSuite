...
class SVGARCHBacktest:
    def __init__(self, data_files):
        ...
        # Stan model codes
        self.sv_models = {
            'sv1': self._get_sv1_code(),
            'sv_t': self._get_sv_t_code(),
            'sv_ma': self._get_sv_ma_code(),
            'sv_l': self._get_sv_l_code()
        }
    
    def _get_sv1_code(self):
        return """
        data {
          int<lower=1> T;
          vector[T] y;
    ...
    def _compile_stan_model(self, name, code):
        """Helper to write and compile Stan models."""
    ...
    def plot_returns_vs_var_for_window(self, series_name, strategy_returns, var_series, 
                                     window_name, window_info, save_dir='sv_garch_backtest_results'):
    ...
    def run_analysis(self):
        ...
        stan_models = {}
        for model_key, model_code in self.sv_models.items():
            print(f"Compiling {model_key} model...")
            stan_models[model_key] = self._compile_stan_model(model_key, model_code)
        ...
        for file_path in self.data_files:
            ...
            try:
                ...
                for model_key, model in stan_models.items():
                    try:
                        fit = model.sample(
                            data={'T': T, 'y': y},
                            chains=4,
                        ...
                        sv_results[model_key] = fit
                        
                        # Save summary statistics
                        summary_df = fit.summary()
                        summary_df.to_excel(f"{save_dir}/{series_name}_{model_key}_summary.xlsx")
                        
                        print(f"\\n-- {model_key} --")
                        print(summary_df.loc[['mu','phi','sigma_h']])
                    except Exception as e:
                        print(f"Error fitting {model_key}: {e}")
                
                # Calculate rolling VaR for the entire period
                print("\\nCalculating rolling 1-year VaR...")
